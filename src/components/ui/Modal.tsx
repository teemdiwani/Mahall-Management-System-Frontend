...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children, size = 'md', footer }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2.5 sm:p-4">
      ...
      />
      {/* Modal */}
      <div className={`relative bg-white rounded-2xl shadow-2xl w-full ${sizeClasses[size]} max-h-[90vh] sm:max-h-[85vh] flex flex-col overflow-hidden`}>
        {/* Header */}
        {title && (
          <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 sm:py-4 border-b border-gray-100">
            <h3 className="text-base font-semibold text-gray-800 truncate pr-2">{title}</h3>
            <button
              onClick={onClose}
        ...
        {/* Footer */}
        {footer && (
          <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-100 flex flex-wrap sm:flex-nowrap justify-end gap-2 sm:gap-3 bg-gray-50/50">
            {footer}
          </div>
        )}
      </div>
